...
export const AgentTable: React.FC<AgentTableProps> = ({ agents, onManage }) => {
  return (
        ...
        <tbody className="divide-y divide-gray-200">
          {agents.map((agent) => (
            <tr key={agent.id} className="hover:bg-gray-50">
              <td className="px-6 py-4 text-gray-900 font-medium">{agent.full_name || 'Unnamed Agent'}</td>
              <td className="px-6 py-4 text-gray-600">{agent.mobile}</td>
              <td className="px-6 py-4 text-gray-600">Joined Feb 2026</td>
